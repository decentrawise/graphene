...
{ try {
   ...
   account_balance_id_type obj_id = obj.get_id();
   ...
   BOOST_CHECK_NE((long)db.find(obj_id), (long)nullptr);
} FC_LOG_AND_RETHROW() }
...
{ try {
   ...
   {
      const account_object* aptr = dynamic_cast< const account_object* >( my_accounts.find( object_id_type( account_id_type( i ) ) ) );
      if( aptr )
      {
         count++;
...
BOOST_AUTO_TEST_CASE( required_approval_index_test )
{ try {
