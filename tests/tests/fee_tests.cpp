...
BOOST_AUTO_TEST_CASE( non_core_fee_refund_test )
{
   try
   {
      ...
      for( int i=0; i<2; i++ )
      {
         ...
         int64_t usd_fee_refund_core = order_create_fee;
         int64_t usd_fee_refund_usd = 0;
...
BOOST_AUTO_TEST_CASE( bsip26_fee_refund_test )
{
   try
   {
      ...
      for( int i=0; i<8; i++ )
      {
         ...
         bool after_bsip26 = ( i >= 4 );
         idump( (after_bsip26)(expire_order)(high_cancel_fee) );
         if( i == 4 )
         {
