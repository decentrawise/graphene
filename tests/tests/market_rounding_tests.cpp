 ...
 *  requested price the resulting payout would be 0 to one party,
 *  though resulting in something-for-nothing protection triggered
 */
BOOST_AUTO_TEST_CASE( trade_amount_would_be_zero )
{
   try {
...
/***
 * Something for nothing protection when matching a limit order with a call order.
...
/***
 * Another test case for something for nothing protection when matching a limit order with a call order.
 * Also detects the cull_small issue in check_call_orders.
 *
...
/***
 * Yet another test case for something for nothing protection when matching a limit order with a call order.
 * Also detects the cull_small issue in check_call_orders.
 *
...
/***
 * This test case tests one of the scenarios described in bitshares-core issue #343 after hard fork:
 ...
 */
BOOST_AUTO_TEST_CASE( limit_call_rounding_test2_after_hf_343 )
{ try {
   auto mi = db.get_global_properties().parameters.maintenance_interval;
