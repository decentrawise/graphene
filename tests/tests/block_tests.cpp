...
genesis_state_type make_genesis() {
   ...
   genesis_state.immutable_parameters.min_validator_count = INITIAL_VALIDATOR_COUNT;
...
BOOST_AUTO_TEST_CASE( generate_empty_blocks )
{
   try {
      ...
      uint32_t last_block;
      {
         ...
         b = db.generate_block(db.get_slot_time(1), db.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
   ...
   try {
      fc::temp_directory data_dir( graphene::utilities::temp_directory_path() );
      {
         ...
         for( uint32_t i = 0; i < 5; ++i )
         {
            ...
            auto b = db.generate_block( now, db.get_scheduled_validator( 1 ), init_account_priv_key, database::skip_nothing );
         }
   ...
   try {
...

      {
         ...
         for( uint32_t i = 0; i < 30; ++i )
         {
            ...
            auto next_validator = db.get_scheduled_validator( 1 );
...
BOOST_AUTO_TEST_CASE( fork_blocks )
{
   try {
      ...
      {
         auto b = db1.generate_block(db1.get_slot_time(1), db1.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
         try {
            PUSH_BLOCK( db2, b );
      ...
      for( uint32_t j = 0; j <= 4; j += 4 )
      {
         ...
         {
            BOOST_TEST_MESSAGE( i );
            auto b =  db2.generate_block(db2.get_slot_time(next_slot), db2.get_scheduled_validator(next_slot), init_account_priv_key, database::skip_nothing);
            next_slot = 1;
         ...
         {
            auto b = db2.generate_block(db2.get_slot_time(1), db2.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
            good_block = b;
      ...
      {
         db1.generate_block(db1.get_slot_time(1), db1.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
      }
...
BOOST_AUTO_TEST_CASE( out_of_order_blocks )
{
   try {
      ...
      auto init_account_priv_key  = fc::ecc::private_key::regenerate(fc::sha256::hash(string("null_key")) );
      auto b1 = db1.generate_block(db1.get_slot_time(1), db1.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
      auto b2 = db1.generate_block(db1.get_slot_time(1), db1.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
      auto b3 = db1.generate_block(db1.get_slot_time(1), db1.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
      auto b4 = db1.generate_block(db1.get_slot_time(1), db1.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
      auto b5 = db1.generate_block(db1.get_slot_time(1), db1.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
      auto b6 = db1.generate_block(db1.get_slot_time(1), db1.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
      auto b7 = db1.generate_block(db1.get_slot_time(1), db1.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
      auto b8 = db1.generate_block(db1.get_slot_time(1), db1.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
      auto b9 = db1.generate_block(db1.get_slot_time(1), db1.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
      auto b10 = db1.generate_block(db1.get_slot_time(1), db1.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
      auto b11 = db1.generate_block(db1.get_slot_time(1), db1.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
      auto b12 = db1.generate_block(db1.get_slot_time(1), db1.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
   ...
   try {
      fc::temp_directory data_dir( graphene::utilities::temp_directory_path() );
      {
         ...
         t.amount = asset( 10000000 );
         {
            ...
            auto b = db.generate_block(db.get_slot_time(1), db.get_scheduled_validator(1), init_account_priv_key, ~0);
         }
         ...
         auto b = db.generate_block(db.get_slot_time(1), db.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
...
BOOST_AUTO_TEST_CASE( switch_forks_undo_create )
{
   try {
      ...
      b = db2.generate_block(db2.get_slot_time(1), db2.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
      db1.push_block(b);
...
BOOST_AUTO_TEST_CASE( duplicate_transactions )
{
   try {
      ...
      auto b = db1.generate_block( db1.get_slot_time(1), db1.get_scheduled_validator( 1 ), init_account_priv_key, skip_sigs );
...
BOOST_AUTO_TEST_CASE( tapos )
{
   try {
      ...
      trx.sign( init_account_priv_key, db1.get_chain_id() );
      PUSH_TX(db1, trx);
      b = db1.generate_block(db1.get_slot_time(1), db1.get_scheduled_validator(1), init_account_priv_key, database::skip_nothing);
      trx.clear();
...
BOOST_FIXTURE_TEST_CASE( rsf_missed_blocks, database_fixture )
{
   try
   {
	 ...
	 fc::uint128_t rsf = db.get_dynamic_global_properties().recent_slots_filled;
      ...
      BOOST_CHECK_EQUAL( db.validator_participation_rate(), (uint32_t)GRAPHENE_100_PERCENT );
      ...
      BOOST_CHECK_EQUAL( db.validator_participation_rate(), pct(127) );
      ...
      BOOST_CHECK_EQUAL( db.validator_participation_rate(), pct(126) );
      ...
      BOOST_CHECK_EQUAL( db.validator_participation_rate(), pct(124) );
      ...
      BOOST_CHECK_EQUAL( db.validator_participation_rate(), pct(121) );
      ...
      BOOST_CHECK_EQUAL( db.validator_participation_rate(), pct(116) );
      ...
      BOOST_CHECK_EQUAL( db.validator_participation_rate(), pct(108) );
      ...
      BOOST_CHECK_EQUAL( db.validator_participation_rate(), pct(95) );
      ...
      BOOST_CHECK_EQUAL( db.validator_participation_rate(), pct(31) );
      ...
      BOOST_CHECK_EQUAL( db.validator_participation_rate(), pct(8) );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_FIXTURE_TEST_CASE( transaction_invalidated_in_cache, database_fixture )
{
   try
   {
      ...
      {
         return d.generate_block(d.get_slot_time(1), d.get_scheduled_validator(1), init_account_priv_key, skip);
      };
...
{ try {
   ...
   generate_blocks( validator_schedule_id_type()(db).current_shuffled_validators.size() );
   generate_blocks( db.get_dynamic_global_properties().next_maintenance_time );

   std::vector<validator_id_type> validators = validator_schedule_id_type()(db).current_shuffled_validators;
   BOOST_CHECK_EQUAL( INITIAL_VALIDATOR_COUNT, validators.size() );
...
BOOST_FIXTURE_TEST_CASE( miss_many_blocks, database_fixture )
{
   try
   {
      ...
      auto get_misses = []( database& db ) {
         ...
         for( const auto& validator_id : validator_schedule_id_type()(db).current_shuffled_validators )
...
{
   try {
      size_t num_validators = db.get_global_properties().block_producers.size();
      size_t dmin = num_validators >> 1;
      ...
      cur_round.reserve( num_validators );
      full_schedule.reserve( total_blocks );
      cur_round.push_back( db.get_dynamic_global_properties().current_validator );

      // we assert so the test doesn't continue, which would
      // corrupt memory
      assert( num_validators <= validator_seen.size() );
      ...
      while( full_schedule.size() < total_blocks )
      {
         ...
         validator_id_type wid = db.get_scheduled_validator( 1 );
         full_schedule.push_back( wid );
         cur_round.push_back( wid );
         if( cur_round.size() == num_validators )
         {
...
BOOST_FIXTURE_TEST_CASE( block_size_test, database_fixture )
{
   try
   {
      ...
      for( uint64_t i = 90; i <= 230; ++i )
      {
         ...
         maybe_large_block.validator = db.get_scheduled_validator(1);
         ...
         auto good_block = db.generate_block( db.get_slot_time(1), db.get_scheduled_validator(1), key, database::skip_nothing );
