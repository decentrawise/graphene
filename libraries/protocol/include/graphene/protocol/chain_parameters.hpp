...
namespace graphene { namespace protocol {
   ...
   struct chain_parameters
   {
      ...
      uint32_t    council_proposal_review_period      = GRAPHENE_DEFAULT_COUNCIL_PROPOSAL_REVIEW_PERIOD_SEC; ///< minimum time in seconds that a proposed transaction requiring council authority may not be signed, prior to expiration
      ...
      uint16_t    maximum_producer_count              = GRAPHENE_DEFAULT_MAX_PRODUCERS; ///< maximum number of block producers
      uint16_t    maximum_council_count               = GRAPHENE_DEFAULT_MAX_COUNCIL; ///< maximum number of council delegates
      ...
      share_type  producer_pay_per_block              = GRAPHENE_DEFAULT_PRODUCER_PAY_PER_BLOCK; ///< CORE to be allocated to validators (per block)
      uint32_t    producer_pay_vesting_seconds        = GRAPHENE_DEFAULT_PRODUCER_PAY_VESTING_SECONDS; ///< vesting_seconds parameter for validator VBO's
      share_type  worker_budget_per_day               = GRAPHENE_DEFAULT_WORKER_BUDGET_PER_DAY; ///< CORE to be allocated to workers (per day)
      uint16_t    max_predicate_opcode                = GRAPHENE_DEFAULT_MAX_ASSERT_OPCODE; ///< predicate_opcode must be less than this number
      share_type  fee_liquidation_threshold           = GRAPHENE_DEFAULT_FEE_LIQUIDATION_THRESHOLD; ///< value in CORE at which accumulated fees in blockchain-issued backed assets should be liquidated
