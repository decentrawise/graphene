...
namespace graphene { namespace protocol {
   ...
   struct chain_parameters
   {
      ...
      uint32_t    council_proposal_review_period      = GRAPHENE_DEFAULT_COUNCIL_PROPOSAL_REVIEW_PERIOD; ///< minimum time in seconds that a proposed transaction requiring council authority may not be signed, prior to expiration
      uint32_t    maximum_transaction_size            = GRAPHENE_DEFAULT_MAX_TRANSACTION_SIZE; ///< maximum allowable size in bytes for a transaction
      uint32_t    maximum_block_size                  = GRAPHENE_DEFAULT_MAX_BLOCK_SIZE; ///< maximum allowable size in bytes for a block
      uint32_t    maximum_time_until_expiration       = GRAPHENE_DEFAULT_MAX_TIME_UNTIL_EXPIRATION; ///< maximum lifetime in seconds for transactions to be valid, before expiring
      uint32_t    maximum_proposal_lifetime           = GRAPHENE_DEFAULT_MAX_PROPOSAL_LIFETIME; ///< maximum lifetime in seconds for proposed transactions to be kept, before expiring
      ...
      amount_type cashback_vesting_threshold          = GRAPHENE_DEFAULT_CASHBACK_VESTING_THRESHOLD; ///< the maximum cashback that can be received without vesting
      bool        count_non_member_votes              = GRAPHENE_DEFAULT_COUNT_NON_MEMBER_VOTES; ///< set to false to restrict voting privileges to member accounts
      bool        allow_non_member_whitelists         = GRAPHENE_DEFAULT_ALLOW_NON_MEMBER_WHITELISTS; ///< true if non-member accounts may set whitelists and blacklists; false otherwise
      ...
      amount_type fee_liquidation_threshold           = GRAPHENE_DEFAULT_FEE_LIQUIDATION_THRESHOLD; ///< value in CORE at which accumulated fees in blockchain-issued backed assets should be liquidated
